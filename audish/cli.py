...
def schedule(app, fac, mapping_file, rules_file, out_schedule, out_conflicts, out_metrics):
    ...
    try:
        ...
        # Write conflicts (preserves all original columns for easier identification)
        write_conflicts_excel(out_conflicts, conflicts, original_columns)
...
def generate_metrics(
    ...
    # Conflicts by reason
    if conflicts:
        reason_counts = Counter(c.get('_ReasonCode', 'UNKNOWN') for c in conflicts)
